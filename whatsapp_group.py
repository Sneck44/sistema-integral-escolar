    ...
    def whatsapp_group():
        ...
        for s in students:
            ...
            if len(local_number) == 10 and len(wa_number) == 12:
                valid.append((s, local_number, wa_number))
            else:
                missing.append(s)

        rows = ''
        for s, local_number, wa_number in valid:
            ...
            rows += f'''<tr>
              ...
              <td>{escape(local_number)}</td>
              <td><a class="wa-btn" target="_blank" rel="noopener" href="https://wa.me/{wa_number}?text={text}">Abrir WhatsApp</a></td>
            </tr>'''

        missing_html = ''.join(f'<li>{escape(s.full_name)}</li>' for s in missing)
        numbers_text = ', '.join(local_number for _, local_number, _ in valid)
        body = f'''
        ...
        <div class="card">
          ...
          <p class="muted">Los teléfonos se muestran en formato nacional de 10 dígitos, sin +52. El sistema agrega internamente el código de México únicamente cuando abre un contacto en WhatsApp.</p>
