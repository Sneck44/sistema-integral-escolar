...
def teacher_name_for_active_group():
    ...
    try:
        ...
        access = tgp.TeacherGroupAccess.query.filter_by(grade=grade, group_name=group_name).first()
        if access:
...
def install(app):
    ...
    def account_profile():
        ...
        if request.method == 'POST':
            ...
            welcome_gender = request.form.get('welcome_gender', 'M').strip().upper()
            if len(full_name) < 5:
                flash('Escribe tu nombre completo.')
                return redirect('/account/profile')
            profile.full_name = full_name[:150]
            # Se guarda la preferencia de saludo en sesión para no inferir género por el nombre.
            session['welcome_gender'] = 'F' if welcome_gender == 'F' else 'M'
            core.db.session.commit()
            flash('Tu perfil fue actualizado correctamente.')
            return redirect('/account/profile')
        ...
        note = ('Este será el nombre que aparecerá como docente titular en los documentos y archivos Excel de tu grupo.' if profile.role == 'DOCENTE' else 'Este nombre se utilizará para identificar tu cuenta dentro del sistema.')
        body = f'''
        <div class="page-head"><h1>Mi perfil</h1><p>Actualiza tus datos personales de identificación.</p></div>
        <div class="card" style="max-width:680px">
          <form method="post">
            <label>Nombre completo<input name="full_name" value="{current_name}" maxlength="150" required></label>
            <p class="muted" style="margin:8px 0 18px">{escape(note)}</p>
            <label>Saludo en la pantalla de inicio
              <select name="welcome_gender">
                <option value="M" {"selected" if gender != "F" else ""}>Bienvenido</option>
                <option value="F" {"selected" if gender == "F" else ""}>Bienvenida</option>
              </select>
            </label><br><br>
            <label>Rol<input value="{role}" disabled></label>
            <div style="margin-top:18px"><button style="width:auto">Guardar mi perfil</button></div>
          </form>
        </div>'''
    ...
    def personalized_identity_ui(response):
        ...
        welcome = 'Bienvenida' if session.get('welcome_gender') == 'F' else 'Bienvenido'

        import re
        html = re.sub(r'<h1>¡Bienvenid[oa],\s*[^<]+!</h1>', f'<h1>¡{welcome}, {display_name}!</h1>', html, count=1)
        html = html.replace('<h1>¡Bienvenido, Administrador!</h1>', f'<h1>¡{welcome}, {display_name}!</h1>')
