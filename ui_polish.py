...
:root{--ui-radius:18px;--ui-radius-sm:12px;--ui-ease:cubic-bezier(.2,.7,.2,1);--ui-hover:#a31d3a;--ui-hover-dark:#5a0b1b;--ui-gold:#caa45f}
...
/* Botones: cambio de color claramente visible al acercar el cursor */
...
a[href*="/delete"]:hover{
  ...
  box-shadow:0 10px 24px rgba(180,35,47,.22)!important;
}
button:active,.action-btn:active,.btn:active{transform:translateY(0) scale(.985)}

/* Menú lateral también responde con color */
.nav-link{transition:background-color .20s var(--ui-ease),transform .18s var(--ui-ease),color .18s var(--ui-ease)!important}
...
.quick a{
  min-height:118px!important;border:1px solid rgba(123,16,36,.08)!important;border-radius:var(--ui-radius)!important;
  text-decoration:none;background:linear-gradient(180deg,#fff 0%,#fdfbfa 100%)!important;
  box-shadow:0 7px 20px rgba(63,22,33,.055)!important;position:relative;overflow:hidden;
  transition:transform .22s var(--ui-ease),box-shadow .22s var(--ui-ease),border-color .22s var(--ui-ease),background .22s var(--ui-ease),color .22s var(--ui-ease)!important;
}
...
.quick a:hover{
  ...
  background:linear-gradient(145deg,#7b1024 0%,#a31d3a 100%)!important;
  color:#fff!important;border-color:#7b1024!important;
  box-shadow:0 16px 32px rgba(74,18,32,.20)!important;
}
.quick a:hover:before{transform:scale(1.35);background:rgba(202,164,95,.20)}
.quick a:hover .quick-icon,.quick a:hover .quick-label,.quick a:hover .quick-sub{color:#fff!important}
.quick a:hover .quick-icon{transform:scale(1.10) rotate(-3deg)}
.quick-icon{transition:transform .22s var(--ui-ease),color .20s var(--ui-ease)}
...
.quick a.rubric-ai-card .quick-icon{width:46px;height:46px;border-radius:50%;display:grid;place-items:center;background:linear-gradient(145deg,#7b1024,#a31d3a);color:#fff!important;box-shadow:0 8px 18px rgba(123,16,36,.20)}
...
.quick a.rubric-ai-card:hover .quick-icon{background:var(--ui-gold)!important;color:#4e0917!important}
...
@media(max-width:720px){.quick{grid-template-columns:repeat(2,minmax(0,1fr))!important;gap:10px!important}.quick a{min-height:102px!important;border-radius:16px!important}}
</style>
'''
...
def install(app):
    ...
    def ui_polish(response):
        ...
        # Sustituye la versión anterior del estilo si algún HTML la conserva y añade la actual.
        if '</head>' in html and 'id="ui-polish-v2"' not in html:
