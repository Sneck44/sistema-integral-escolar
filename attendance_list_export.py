...
def _logo_candidates():
    ...
    return [
        os.path.join(base, 'static', 'encabezado-logos-puebla.png'),
        os.path.join(base, 'static', 'logo-benito-juarez-final.PNG'),
    ]
...
def build_attendance_workbook():
    ...
    ws.set_portrait()
    ws.set_paper(1)
    ws.set_margins(0.31, 0.23, 0.33, 0.02)
    ws.fit_to_pages(1, 1)
    ws.hide_gridlines(2)

    ws.set_column('A:A', 0.7)
    ws.set_column('B:B', 4.2)
    ws.set_column('C:C', 32.4)
    ...
    if os.path.exists(logos[0]):
        try: ws.insert_image('A1', logos[0], {'x_scale':0.48,'y_scale':0.48,'x_offset':2,'y_offset':2})
        except Exception: pass
    if os.path.exists(logos[1]):
        try: ws.insert_image('AA1', logos[1], {'x_scale':0.20,'y_scale':0.20,'x_offset':2,'y_offset':2})
        except Exception: pass
    ...
    ws.merge_range('B13:B14', 'N/P', fmt_head)
    ws.merge_range('C13:C14', '          NOMBRE DEL ALUMNO', fmt_head)
    for col in range(3, 30):
        ws.merge_range(12, col, 13, col, '', fmt_head)
    ...
    for i in range(data_rows):
        ...
        if i < len(students):
            s = students[i]
            number = s.list_no if s.list_no is not None else i + 1
            name = (s.full_name or '').upper()
        else:
            number = ''
            name = ''
        ws.write(row, 1, number, fmt_num)
        ws.write(row, 2, name, fmt_name)
        for col in range(3, 30):
            ws.write_blank(row, col, None, fmt_day)
        ws.set_row(row, 18)

    end_data = first_row + data_rows - 1
    date_row = end_data + 2
    teacher_role_row = date_row + 2
    director_role_row = teacher_role_row + 1
    line_row = teacher_role_row + 3
    name_row = line_row + 1
    ...
    ws.merge_range(line_row, 2, line_row, 9, '___________________________________________', fmt_sig)
    ws.merge_range(line_row, 11, line_row, 29, '_______________________________________', fmt_sig)
    ws.merge_range(name_row, 2, name_row, 9, _teacher_name().upper(), fmt_sig_name)
